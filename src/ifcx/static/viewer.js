...
const root = document.getElementById('ifcx-root');
const canvas = document.getElementById('ifcx-canvas');
const statusNode = document.getElementById('ifcx-status');
const panel = document.getElementById('ifcx-panel');
const panelContent = document.getElementById('panel-content');
const modelPanel = document.getElementById('ifcx-model-panel');
const modelSearch = document.getElementById('ifcx-model-search');
const modelTree = document.getElementById('ifcx-model-tree');
const contextMenu = document.getElementById('ifcx-context-menu');
...
function buildBatch(name, sections, transparent) {
  ...
  const material = new THREE.MeshStandardMaterial({
    ...
    // Alpha hashing keeps dynamic semi-transparent colors usable even when an
    // occurrence started in the opaque batch, without adding another draw call.
...
function renderElementGroup({ groupKey, label, focusLabel, elements, depth, defaultOpen = false }) {
  ...
  connectGroupToggle({ details, summary, children, populate, groupKey, elements });
...
function setFocusMaterialMode(enabled) {
  for (const batch of state.batches.values()) {
    ...
    batch.material.alphaHash = enabled ? false : !batch.transparent;
    batch.material.depthWrite = enabled ? false : !batch.transparent;
...
function setStatus(text, warning = false) {
  ...
  setStatus.timer = setTimeout(() => statusNode.classList.remove('visible'), warning ? 8000 : 3500);
}
